...
public class LocalEntityManagerFactoryListener implements ServletContextListener{
	...
	private void initAdminUserAndRuoli() throws Exception {
		...
		if (ruoloServiceInstance.cercaPerDescrizioneECodice("Pizzaiolo User", "PIZZAIOLO_USER") == null) {
			ruoloServiceInstance.inserisciNuovo(new Ruolo("Pizzaiolo User", "PIZZAIOLO_USER"));
		}
		
		if (ruoloServiceInstance.cercaPerDescrizioneECodice("fattorino User", "FATTORINO_USER") == null) {
			ruoloServiceInstance.inserisciNuovo(new Ruolo("fattorino User", "FATTORINO_USER"));
		}
		...
		if (utenteServiceInstance.findByUsernameAndPassword("pizzaiolouser", "pizzaiolouser") == null) {
			Utente pizzaiolo = new Utente("pizzaiolouser", "pizzaiolouser", "Franco", "Cossi", new Date());
			...
			utenteServiceInstance.aggiungiRuolo(pizzaiolo,
					ruoloServiceInstance.cercaPerDescrizioneECodice("Pizzaiolo User", "PIZZAIOLO_USER"));
		}
		
		if (utenteServiceInstance.findByUsernameAndPassword("fattorinouser", "fattorinouser") == null) {
			Utente fattorino = new Utente("fattorinouser", "fattorinouser", "Giovanni", "Forte", new Date());
			...
			utenteServiceInstance.aggiungiRuolo(fattorino,
					ruoloServiceInstance.cercaPerDescrizioneECodice("fattorino User", "FATTORINO_USER"));
		}
	}
